# Claude Code Project Instructions — tokio-util-kotlin

## Project Overview

This is **tokio-util-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tokio-util`](https://crates.io/crates/tokio-util).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tokio-util/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tokio-util>
